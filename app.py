...
app.py
------
Spark - Your Personal AI Career Advisor.
...
""",
    ...
    with cta_col:
        if st.button("🚀 Start Analyzing", type="primary", use_container_width=True):
            st.session_state.spark_started = True
            st.rerun()

else:
    # =====================================================================
    # MAIN DASHBOARD (unchanged functionality below this point)
    # =====================================================================
    ...
    skills_tracked = len(SKILL_KEYWORDS)

    overview_cards = [
        ("📊", "Jobs Analyzed", f"{total_jobs:,}"),
        ("💼", "Unique Career Roles", f"{unique_titles:,}"),
        ("🛠️", "Skills Tracked", f"{skills_tracked:,}"),
    ]
    ...
    with st.sidebar:
        ...
        with st.container(border=True):
            st.markdown("**⚡ Project**")
            st.caption("Spark - Your Personal AI Career Advisor")
            st.markdown("**📚 Subject**")
    ...
    if analyze_clicked:
        ...
        if filtered_df.empty:
            ...
            with tab_market:
                ...
                with grid_col2:
                    with st.container(border=True):
                        section_heading("🧭", "Job Domain Distribution")
                        # "Not Specified" is a missing-data placeholder (see analysis.load_data),
                        # not a real domain, so it's excluded here the same way the rest of the
                        # app treats it - this keeps the chart limited to genuine signal.
                        domain_counts = (
                            filtered_df.loc[filtered_df["job_domain"].str.lower() != "not specified", "job_domain"]
                            .value_counts()
                            .head(6)
                            .reset_index()
                        )
                        ...
                        if domain_counts.empty:
                            st.info("No job domain data is available for the matching job postings.")
                        else:
                            st.caption("Which broader industry areas these postings fall under.")
                            fig_domains = px.bar(
                                domain_counts,
                                x="Count",
                                y="Job Domain",
